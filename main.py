...
# --- 👥 ABONNÉS ---
# Tous les utilisateurs ici recevront les alertes (car le filtre est strict)
SUBSCRIBERS = [
    { "name": "Administrateur", "id": "1952904877" },
    { "name": "Membre 2",       "id": "7879373928" },
    { "name": "Membre 3",       "id": "8260779046" }
]

# --- 🎯 WHITELIST ACHETEURS (VERSION COMPLÈTE) ---
TARGET_BUYERS = [
    ...
    # Cas spécifiques de fautes fréquentes :
    ...
    "INSTITUT DES TECHNICIENS",      # ITSA / ITA
...
def scan_ao_attempt():
    ...
    with sync_playwright() as p:
        ...
        log(f"🌍 Connexion AO...")
        log(f"📅 Filtre Pub : {date_pub_start} -> {date_pub_end}")
        ...
        try:
            ...
            try:
                ...
                log(f"📊 Total : {total_results} offres.")
            except:
                total_results = 0

            if total_results > 10:
                log("🔄 Passage à 500 résultats...")
                try:
            ...
            for current_page in range(1, total_pages + 1):
                log(f"📄 Page {current_page}/{total_pages}...")
    ...
    # --- ENVOI MULTI-UTILISATEURS ---
    if pending_alerts:
        count_sent = 0
        for item in pending_alerts:
            new_ids.add(item['id'])
            # Boucle sur TOUS les abonnés de la liste
            for sub in SUBSCRIBERS:
                send_telegram_to_user(sub["id"], item['msg'])
            count_sent += 1
        
        seen_ids.update(new_ids)
        save_seen(seen_ids)
        log(f"🚀 {count_sent} alertes envoyées à {len(SUBSCRIBERS)} utilisateurs.")
    else:
...
if __name__ == "__main__":
    log("🚀 Bot AO Démarré (MULTI-USER)")
    # Notification de démarrage à tous
    for sub in SUBSCRIBERS:
        send_telegram_to_user(sub["id"], "🚜 Bot AO (Strict) : Démarrage du service de surveillance Agri !")
